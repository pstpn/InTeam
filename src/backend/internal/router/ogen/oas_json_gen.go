...
// Encode implements json.Marshaler.
func (s *FeedbackWithUsername) Encode(e *jx.Encoder) {
	e.ObjStart()
	s.encodeFields(e)
	e.ObjEnd()
}
...
// encodeFields encodes fields.
func (s *FeedbackWithUsername) encodeFields(e *jx.Encoder) {
	{
		e.FieldStart("date")
		json.EncodeDateTime(e, s.Date)
	}
...
var jsonFieldsNameOfFeedbackWithUsername = [6]string{
	0: "date",
	1: "feedback",
	2: "racket_id",
	3: "rating",
...
// Decode decodes FeedbackWithUsername from json.
func (s *FeedbackWithUsername) Decode(d *jx.Decoder) error {
	if s == nil {
		return errors.New("invalid: unable to decode FeedbackWithUsername to nil")
	}
	var requiredBitSet [1]uint8
	...
	if err := d.ObjBytes(func(d *jx.Decoder, k []byte) error {
		...
		return errors.Wrap(err, "decode FeedbackWithUsername")
	}
	// Validate required fields.
	var failures []validate.FieldError
	for i, mask := range [1]uint8{
		...
		if result := (requiredBitSet[i] & mask) ^ mask; result != 0 {
			...
			for bitN := 0; bitN < missed; bitN++ {
				...
				fieldIdx := i*8 + bitIdx
				var name string
				if fieldIdx < len(jsonFieldsNameOfFeedbackWithUsername) {
					name = jsonFieldsNameOfFeedbackWithUsername[fieldIdx]
				} else {
					name = strconv.Itoa(fieldIdx)
				}
				failures = append(failures, validate.FieldError{
...
// MarshalJSON implements stdjson.Marshaler.
func (s *FeedbackWithUsername) MarshalJSON() ([]byte, error) {
	e := jx.Encoder{}
	s.Encode(&e)
	return e.Bytes(), nil
}
...
// UnmarshalJSON implements stdjson.Unmarshaler.
func (s *FeedbackWithUsername) UnmarshalJSON(data []byte) error {
	d := jx.DecodeBytes(data)
	return s.Decode(d)
}
...
func (s *GetRacketFeedbacksResponse) Decode(d *jx.Decoder) error {
	...
	if err := d.ObjBytes(func(d *jx.Decoder, k []byte) error {
		...
		case "feedbacks":
			requiredBitSet[0] |= 1 << 0
			if err := func() error {
				s.Feedbacks = make([]FeedbackWithUsername, 0)
				if err := d.Arr(func(d *jx.Decoder) error {
					var elem FeedbackWithUsername
					if err := elem.Decode(d); err != nil {
						return err
					}
					s.Feedbacks = append(s.Feedbacks, elem)
